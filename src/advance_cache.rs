...
impl AdvanceCache {
    ...
    /// The advance width of one grapheme cluster, shaping and caching it on
    /// first sight.
    fn advance(&self, grapheme: &str) -> f32 {
        if let Some(&w) = self
            .widths
            .lock()
            .expect("lock advance cache")
            .get(grapheme)
        {
            return w;
        }
